...
public class QuestionServiceImpl implements QuestionService {
    ...
    public void edit(long questionId, EditQuestionDto dto, MultipartFile file) throws IOException {
        ...
        if (question.getAnswerType() != editQuestion.getAnswerType()) {
            ...
            if (question.getAnswerType() == AnswerType.MATCH) {
                ...
                List<Question> deletedSubQuestions = questionRepository
                        .findByIdNotInAndSupQuestion(notDeletedIds, question);
                ...
                List<Long> notDeletedIds = editQuestion.getAnswers().stream()
                        ...
                        .filter(Objects::nonNull)
                        .collect(Collectors.toList());
                List<Answer> deletedAnswers = answerRepository
                        .findAnswersByIdNotInAndQuestion(notDeletedIds, question);
            ...
            if (editQuestion.getAnswerType() == AnswerType.MATCH) {
                List<Question> addedSubQuestions = editQuestion.getSubQuestions()
                        .stream()
                        .filter(x -> x.getId() == null)
                        .collect(Collectors.toList());

                List<Question> existedSubQuestions = editQuestion.getSubQuestions()
                        .stream()
                        .filter(x -> x.getId() != null)
                        .collect(Collectors.toList());
                ...
                List<Answer> addedAnswers = editQuestion.getAnswers()
                        .stream()
                        .filter(x -> x.getId() == null)
                        .collect(Collectors.toList());

                List<Answer> existedAnswers = editQuestion.getAnswers()
                        .stream()
                        .filter(x -> x.getId() != null)
                        .collect(Collectors.toList());
    ...
    private void changeCoefficient(List<Question> questions) {
        ...
        for (Question question : questions) {
            ...
            int correctAnswers = questionStatistic.getCorrectAnswers();
            int wrongAnswers = questionStatistic.getWrongAnswers();
            int lastChangeAnswerCount = questionStatistic
                    .getCoefficientChangeAnswersCount();
            if (correctAnswers + wrongAnswers - lastChangeAnswerCount >= 100) {
                ...
                if (correctAnswerCoefficient < coefficientThreshold) {
                    double lastCorrectAnswerCoefficient = question
                            .getLastCorrectAnswerCoefficient();
                    ...
                    question.setLastCorrectAnswerCoefficient(question.getDifficulty()
                            .getCoefficient());
                }
